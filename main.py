...
def gaussian_noise(img):
    noise = np.zeros((img.shape[0], img.shape[1], img.shape[2]), dtype=np.uint8)
    cv.randn(noise,(0,0,0),(0.5,0.5,0.5)) 
    return img + noise

for root in [TRAIN_ROOT,TEST_ROOT,VALIDATE_ROOT]:
    if not (os.path.isdir(root+NOISY_GAUSSIAN)):
        os.makedirs(root+NOISY_GAUSSIAN) 
    ...
    for file in files:
        ...
        cv.imwrite(root+NOISY_GAUSSIAN+file.replace(".jpg", "_noise.jpg" ), noise_img)
'''
